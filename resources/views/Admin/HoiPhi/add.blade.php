...
<div class="row">
	<div class="col-12">
    	<div class="card-box table-responsive">
    		<h3 class="m-t-0"><a href="{{ env('APP_URL') }}admin/hiep-hoi-doanh-nghiep/hoi-phi" class="btn btn-info"><i class="mdi mdi-reply-all"></i></a> Thu Hội Phí Doanh Nghiệp</h3>
    		<form action="{{ env('APP_URL') }}admin/hiep-hoi-doanh-nghiep/hoi-phi/create" method="post" id="dinhkemform" enctype="multipart/form-data">
          {{ csrf_field() }}
          <div class="form-body">
            <hr />
            @if($errors->any())
                          ...
                          <select name="id_doanh_nghiep[]" id="id_doanh_nghiep" class="form-control select2" required data-placeholder="Chọn Doanh nghiệp" multiple>
                            ...
                            @if($doanhnghiep)
                              @foreach($doanhnghiep as $dn)
                                <option value="{{ $dn['_id'] }}" @if($dn['_id'] == old('id_doanh_nghiep')) selected @endif >{{ $dn['ten'] }}</option>
                              @endforeach
                            @endif
                          </select>
                      </div>
            ...
            <div class="row">
                <div class="col-md-12">
                  <div class="form-group row">
                        ...
                        <div class="col-md-4">
                            <input type="text" id="so_tien" name="so_tien" class="form-control number" placeholder="Số tiền" value="{{ old('so_tien') !== null ? old('so_tien') : 2000000}}" required />
                        </div>
                        <label class="control-label col-md-1 text-right p-t-10">{{ __('Ngay thu') }}</label>
                        <div class="col-md-2">
                            <input type="text" id="ngay_thu" name="ngay_thu" class="form-control" placeholder="Ngày thu" value="{{ old('ngay_thu') ? old('ngay_thu') : date("Y-m-d H:i:s") }}" required />
                        </div>
                  </div>
                </div>
              </div>
            <div class="row">
              <div class="col-md-12">
                  <div class="form-group row">
                      ...
                      <div class="col-md-10">
                        <textarea name="noi_dung" id="noi_dung" required placeholder="Nội dung thu" class="form-control">{{ old('noi_dung') ? old('noi_dung') : 'Hội phí thường niên'  }}</textarea>
                      </div>
                  </div>
              </div>
            </div>
