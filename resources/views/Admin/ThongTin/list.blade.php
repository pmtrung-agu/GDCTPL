    ...
    <div class="container-fluid">
        <div class="row">
            <div class="col-12 card-box">
                <h3 class="m-t-0"><a href="{{ env('APP_URL') }}admin/thong-tin/add" class="btn btn-primary btn-sm"><i class="fa fa-plus"></i> {{ __('Thêm mới') }}</a> {{ __('Danh sách Thông tin') }}</h3>
                <hr />
                <form method="GET" action="{{ env('APP_URL') }}admin/thong-tin">
                ...
                <table class="table table-border table-bordered table-striped table-sm">
                    ...
                    @foreach($danhsach as $key => $ds)
                        <tr>
                            ...
                            <td>
                                <a href="{{ env('APP_URL') }}thong-tin-chi-tiet/{{ $ds['slug'] }}" target="_blank"><strong>{{ $ds['ten'] }}</strong></a>
                    ...
                    @if($keywords)
                        {{ $danhsach->withPath(env('APP_URL').'admin/thong-tin?' . $_SERVER['QUERY_STRING']) }}
                    @else
                        {{ $danhsach->withPath(env('APP_URL') . 'admin/thong-tin') }}
                    @endif
                @endif
