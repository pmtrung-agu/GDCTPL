...
<div class="row">
	<div class="col-12">
    	<div class="card-box table-responsive">
    		...
    		<form action="{{ env('APP_URL') }}admin/san-pham/create" method="post" id="dinhkemform" enctype="multipart/form-data">
          {{ csrf_field() }}
          <div class="form-body">
            <hr />
            @if($errors->any())
                          ...
                          <select name="id_product_category[]" multiple="multiple" id="id_product_category" class="form-control select2" required data-placeholder="Chọn loại sản phẩm">
                            ...
                            @if($product_categories)
                              @foreach($product_categories as $cat)
                                <option value="{{ $cat['_id'] }}">{{ $cat['ten'] }}</option>
                              @endforeach
                            @endif
                          </select>
                      </div>
                    ...
                    <div class="col-md-6">
                      <select name="id_seller" id="id_seller" class="form-control select2">
                        @if($sellers)
                          ...
                          @endphp
                            <option value="{{ $seller['_id'] }}" @if($seller['_id'] == $id_user) selected @endif>{{ $seller['username'] }} @if($dn) - {{ $dn['ten'] }} @endif</option>
                          @endforeach
                        @endif
                      </select>
                    </div>
  ...
  <script src="{{ env('APP_URL') }}assets/backend/js/script.js" type="text/javascript"></script>
    ...
    $(document).ready(function() {
      ...
      $("#title").keyup(function(){
        var _this = $(this);
        $.get('{{ env('APP_URL') }}slug/'+$("#code").val()+ " " + $("#title").val(), function(data){
          $("#slug").val(data);
        });
      });
