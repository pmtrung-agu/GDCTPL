...
@section('css')
  <link href="{{ env('APP_URL') }}assets/backend/libs/summernote/summernote-lite.css" rel="stylesheet" />
...
<div class="row">
	<div class="col-12">
    	<div class="card-box table-responsive">
    		<h3 class="m-t-0"><a href="{{ env('APP_URL') }}admin/san-pham" class="btn btn-info"><i class="mdi mdi-reply-all"></i></a> Thêm sản phẩm</h3>
    		<form action="{{ env('APP_URL') }}admin/san-pham/update" method="post" id="dinhkemform" enctype="multipart/form-data">
          {{ csrf_field() }}
                      ...
                      <div class="col-md-10">
                          <select name="id_product_category[]" multiple="multiple" id="id_product_category" class="form-control select2" required data-placeholder="Chọn loại sản phẩm">
                            ...
                            @foreach($product_categories as $cat)
                              <option value="{{ $cat['_id'] }}" @if(in_array($cat['_id'], $product['id_product_category'])) selected @endif>{{ $cat['ten'] }}</option>
                              @if($cat['level_1'])
                                @foreach($cat['level_1'] as $l1)
                                  <option value="{{ $l1['_id'] }}" @if(in_array($l1['_id'], $product['id_product_category'])) selected @endif>---| {{ $l1['ten'] }}</option>
                                    @if($l1['level_2'])
                                      @foreach($l1['level_2'] as $l2)
                                        <option value="{{ $l2['_id'] }}" @if(in_array($l2['_id'], $product['id_product_category'])) selected @endif>---|---| {{ $l2['ten'] }}</option>
                                      @endforeach
                                    @endif
                                @endforeach
                              @endif
                    ...
                    <div class="col-md-6">
                      <select name="id_seller" id="id_seller" class="form-control select2">
                        @if($sellers)
                          ...
                          @endphp
                            <option value="{{ $seller['_id'] }}" @if($seller['_id']==$product['id_seller']) selected @endif>{{ $seller['username'] }} @if($dn) - {{ $dn['ten'] }} @endif</option>
                          @endforeach
                        @endif
                      </select>
                    </div>
...
@section('js')
  ...
  <script src="{{ env('APP_URL') }}assets/backend/libs/summernote/summernote-lite.js"></script>
