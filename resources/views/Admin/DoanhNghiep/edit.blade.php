                    ...
                    <form action="{{ env('APP_URL') }}admin/doanh-nghiep/update" method="post" id="dinhkemform" enctype="multipart/form-data">
                        ...
                        <div class="form-body">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-md-4">
                                    <input type="text" id="ngay_thanh_lap" name="ngay_thanh_lap" class="form-control" placeholder="{{ __('Ngày thành lập') }}" value="{{ $ngay_thanh_lap }}" />
                                </div>
                                            ...
                                            <select class="select2 m-b-10" id="address_1" name="address[]" style="width: 100%" data-placeholder="Chọn Tỉnh">
                                                ...
                                                @if($address)
                                                  @foreach($address as $a)
                                                    <option value="{{ $a['ma'] }}" @if($a['ma'] == $diachi[0]) selected @endif >{{ $a['ten'] }}</option>
                                                  @endforeach
                                                @endif
                                            </select>
                                        </div>
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group row">
                                        <label class="control-label col-md-2 text-right p-t-10">Địa chỉ</label>
                                        <div class="col-md-10">
                                            <input type="text" id="address_4" name="address[]" class="form-control" placeholder="Số nhà, tên đường, khóm, ấp,..." value="{{ $diachi[3]}}" required>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="col-md-2">
                                    <select name="nganhnghe_id" id="nganhnghe_id" class="form-control select2" required data-placeholder="Ngành nghể">
                                        ...
                                        @foreach($nganhnghe as $nn)
                                            <option value="{{ $nn['_id'] }}" @if($nn['_id'] == $nganhnghe_id) selected @endif>{{ $nn['ten'] }}</option>
                                        @endforeach
                                    </select>
                                </div>                                
                                <div class="col-md-4 switchery-demo">
                                    <b>{{ __('Trạng thái') }}: </b>
                                    <input type="checkbox" name="trangthai" id="trangthai" @if($trangthai) checked="checked" @endif class="js-switch" data-plugin="switchery" data-color="#009efb" value="1" @if($trangthai) checked @endif/>
                                    Hội viên HHDN:&nbsp;&nbsp;&nbsp;
                                    <input type="checkbox" name="hoivienhiephoi" id="hoivienhiephoi" @if($hoivienhiephoi) checked="checked" @endif class="js-switch" data-plugin="switchery" data-color="#009efb" value="1"/>
                                </div>
...
@section('js')
    ...
    <script type="text/javascript">
        $(document).ready(function(){
            ...
            @if($diachi[0])
                $.get("{{ env('APP_URL') }}admin/dia-chi/get/{{ $diachi[0] }}/{{ $diachi[1] }}", function(huyen){
                    $("#address_2").html(huyen);
                });
            @endif
            @if($diachi[1])
                $.get("{{ env('APP_URL') }}admin/dia-chi/get/{{ $diachi[1] }}/{{ $diachi[2] }}", function(xa){
                    $("#address_3").html(xa);
                });
            @endif
