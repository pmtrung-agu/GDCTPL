...
@section('title', __('Sửa Thông tin Tài liệu Chuyển đổ số'))
@section('css')
...
<div class="wrapper">
    <div class="container-fluid">
        <div class="row">
            <div class="col-12">
                <div class="card-box">
                    <h3 class="m-t-0"><a href="{{ env('APP_URL') }}admin/tai-lieu" class="btn btn-primary btn-sm"><i class="mdi mdi-reply-all"></i> {{ __('Trở về') }}</a> {{ __('Sửa Tài liệu Giáo dục Chính trị - Pháp luật') }}</h3>
                    <form action="{{ env('APP_URL') }}admin/tai-lieu/update" method="post" id="dinhkemform" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        ...
                        <div class="form-body">
                            ...
                            <div class="form-group row">
                                <label class="control-label col-md-2 text-right p-t-10">{{ __(key: 'Phân loại') }}</label>
                                @php
                                ...
                                <div class="col-md-10">
                                    <select name="tags[]" id="tags" class="form-control select2" multiple required>
                                        ...
                                        @foreach($tags as $ktag => $vtag)
                                            <option value="{{ $vtag['slug'] }}" @if(in_array($vtag['slug'], $t)) selected @endif>{{ $vtag['ten'] }}</option>
                                            @php
                                            ...
                                            @if($childs)
                                                @foreach($childs as $c)
                                                    <option value="{{ $c['slug'] }}" @if(in_array($c['slug'], $t)) selected @endif>   |--- {{ $c['ten'] }}</option>
                                                @endforeach
                                            @endif
                                        @endforeach
                                    </select>
                            ...
                            <div class="form-group row">
                                <label class="control-label col-md-2 text-right p-t-10">{{ __(key: 'Ngày tạo') }}</label>
                                <div class="col-md-2">
                        ...
                        <div class="form-actions">
                            <a href="{{ env('APP_URL') }}admin/tai-lieu" class="btn btn-light"><i class="fa fa-reply-all"></i> {{ __('Trở về') }}</a>
